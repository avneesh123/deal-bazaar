...
export type ClipId = "clip-another1" | "clip-another2" | "clip-another3";
...
export function ClipPathDefs() {
  return (
    <svg className="clipppy absolute -top-[999px] -left-[999px] w-0 h-0" aria-hidden>
      <defs>
...
export const CLIP_IDS: ClipId[] = ["clip-another1", "clip-another2", "clip-another3"];

export function clipForIndex(i: number): ClipId {
  return CLIP_IDS[i % CLIP_IDS.length];
}
...
const ClippedShapeGallery = React.forwardRef<HTMLElement, ClippedShapeGalleryProps>(
  ({ mediaItems, className, ...props }, ref) => {
    const defaultMediaItems: MediaItem[] = [
      ...
      {
        src: "https://images.unsplash.com/photo-1600185365483-26d7a4cc7519?w=800&auto=format&fit=crop",
        alt: "Sneaker side",
        clipId: "clip-another2",
        type: "image",
      },
      {
        src: "https://images.unsplash.com/photo-1551107696-a4b0c5a0d9a2?w=800&auto=format&fit=crop",
        alt: "Sneaker top",
            ...
            <figure key={index} style={{ clipPath: `url(#${item.clipId})` }}>
              {item.type === "image" ? (
                ...
                <img
                  src={item.src}
                  alt={item.alt}
                  className="transition-all duration-500 aspect-[4/5] min-h-full align-bottom object-cover hover:scale-105 w-full"
                />
              ) : (
                <video autoPlay muted loop playsInline className="transition-all duration-500 aspect-[4/5] min-h-full align-bottom object-cover hover:scale-105 w-full">
