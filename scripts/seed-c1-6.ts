/**
 * Seed Batch C1-6: Adidas AE 1 Cloud White (PRE-OWNED) — Size US 8.
 * $51 Whatnot pull from fragmented_soles ("$1 RAPID FIRE SNEAKER AUCTIONS —
 * FREE SHOES ALL STREAM", 16 Oct 2025). Lot BX949. Pre-owned with original box.
 */
...
const FOLDER = "/tmp/db-batchC/c1-6-ae1-cloud-white-sz8";
const SLUG = "adidas-ae-1-cloud-white-sz8-pre-owned";
const NAME = "Adidas AE 1 Cloud White — Pre-Owned (Size 8)";
...
async function main() {
  ...
  const heic = path.join(FOLDER, "_receipt", "IMG_5762.HEIC");
  if (existsSync(heic)) {
    console.log("\n🧾 uploading receipt...");
    ...
    if (error) {
      ...
      const { data: receipt, error: rErr } = await supabase
        ...
        .insert({
          ...
          notes:
            "Whatnot packing slip — order #586293048 from fragmented_soles' '$1 RAPID FIRE SNEAKER AUCTIONS — FREE SHOES ALL STREAM' livestream on 16 October 2025. Listed as 'ADIDAS AE 1 CLOUD WHITE — SIZE 8 PRE OWNED — BX949 ORIGINAL BOX'. Won for $51.",
        })
        .select("id")
        .single();
      if (!rErr && receipt) {
