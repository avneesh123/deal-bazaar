/**
 * Seed Batch D1-1: Air Jordan 1 Retro High OG Sail — Size 11.
 * Second physical pair of the model already at B6-2 (sz 11). This one was a
 * Whatnot pull — much lower cost basis ($86) — so it lives as a separate
 * listing with its own SKU rather than bumping B6-2's quantity.
 *
 * $86 Whatnot pull from stewsshoes ("NO MISSES, JUST HEATERS AT A $1.",
 * 16 Oct 2025). Lot #8. Brand new with OG box (box may show minor damage).
 */
...
const FOLDER = "/tmp/db-batchD/d1-1-aj1-sail-sz11";
const SLUG = "air-jordan-1-retro-high-og-sail-sz11-d1";
const NAME = "Air Jordan 1 Retro High OG Sail (Size 11)";
...
async function main() {
  ...
  const heic = path.join(FOLDER, "_receipt", "IMG_5772.HEIC");
  if (existsSync(heic)) {
    console.log("\n🧾 uploading receipt...");
    ...
    if (error) {
      ...
      const { data: receipt, error: rErr } = await supabase
        ...
        .insert({
          ...
          notes:
            "Whatnot packing slip — order #583645632 from stewsshoes' 'NO MISSES, JUST HEATERS AT A $1.' livestream on 16 October 2025. Listed as 'JORDAN 1 RED SAIL — SIZE 11 #8 — BRAND NEW OG BOX, BOX MIGHT BE DAMAGED'. Won for $86. Same model as B6-2 but separate physical pair / much lower cost basis.",
        })
        .select("id")
        .single();
      if (!rErr && receipt) {
