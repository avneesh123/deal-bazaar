/**
 * Seed Batch C1-2: Air Jordan 1 Retro High OG Mauve — Size 10.
 * $80 Whatnot pull from stewsshoes ("NO MISSES, JUST HEATERS AT A $1." livestream, 16 Oct 2025).
 * Slip lot #28. Brand new with OG box. Different size from B4-1/B5-4 (both sz 10.5).
 *
 * Catalog images are AI-cleaned PNGs (Picsart) the user dropped in — no bg-removal needed.
 */
...
const FOLDER = "/tmp/db-batchC/c1-2-aj1-mauve-sz10";
const SLUG = "nike-air-jordan-1-retro-high-og-mauve-sz10";
const NAME = "Nike Air Jordan 1 Retro High OG Mauve (Size 10)";
...
async function main() {
  ...
  const { data: product, error: prodErr } = await supabase
    ...
    .insert({
      ...
      category: "sneakers",
      description:
        "Air Jordan 1 Retro High OG in the Mauve colorway — white leather toe and quarter panels with dusty mauve overlays on the Swoosh, collar, eyestay, and heel. White midsole, white rubber outsole, classic red NIKE AIR tongue tag, mauve laces. Brand new with original Nike kraft box. Size US 10 (EU 44 / UK 9 / 28 cm).",
  ...
  const heic = path.join(FOLDER, "_receipt", "IMG_5744.HEIC");
  if (existsSync(heic)) {
    console.log("\n🧾 uploading receipt...");
    ...
    if (error) {
      ...
      const { data: receipt, error: rErr } = await supabase
        ...
        .insert({
          ...
          notes:
            "Whatnot packing slip — order #583699139 from stewsshoes' 'NO MISSES, JUST HEATERS AT A $1.' livestream on 16 October 2025. Listed as 'JORDAN 1 MAUVE — SIZE 10 #28 — BRAND NEW OG BOX'. Won for $80.",
        })
        .select("id")
        .single();
      if (!rErr && receipt) {
