/**
 * Seed Batch D1-5: Nike NOCTA Air Zoom Drive Summit White — Size US 12.
 * $50 Whatnot pull #6 from stewsshoes ("NO MISSES, JUST HEATERS AT A $1.",
 * 16 Oct 2025). Brand new with OG box. Same model as B5-1 in a different
 * size (B5-1 is sz 11), not a duplicate.
 */
...
const FOLDER = "/tmp/db-batchD/d1-5-nocta-zoom-drive-sz12";
const SLUG = "nike-nocta-air-zoom-drive-summit-white-sz12";
const NAME = "Nike NOCTA Air Zoom Drive Summit White (Size 12)";
...
async function main() {
  ...
  const { data: product, error: prodErr } = await supabase
    ...
    .insert({
      ...
      category: "sneakers",
      description:
        "Nike x NOCTA Air Zoom Drive in the Summit White colorway — Drake's NOCTA collab take on a chunky-soled trainer. Tonal white mesh and synthetic upper, sculpted Summit White midsole, NOCTA woven heel pull tab, and the NOCTA snowflake mark on the lateral side. Brand new with original Nike box. Size US 12 (W 13.5 / UK 11 / EU 46 / 30 cm).",
  ...
  const heic = path.join(FOLDER, "_receipt", "IMG_5786.HEIC");
  if (existsSync(heic)) {
    console.log("\n🧾 uploading receipt...");
    ...
    if (error) {
      ...
      const { data: receipt, error: rErr } = await supabase
        ...
        .insert({
          ...
          notes:
            "Whatnot packing slip — order #583662030 from stewsshoes' 'NO MISSES, JUST HEATERS AT A $1.' livestream on 16 October 2025. Listed as 'Nike Air Zoom Drive Drake NOCTA Summit White — SZ: 12M/13.5W #6 — Brand New X5GE135N4F'. Won for $50.",
        })
        .select("id")
        .single();
      if (!rErr && receipt) {
