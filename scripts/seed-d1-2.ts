/**
 * Seed Batch D1-2: Nike Wmns Legend Essential 2 Light Soft Pink — W 9.5.
 * $23 random pull from fragmented_soles on Whatnot ("500+ PAIRS AT $1 + FREE
 * SHOES", 15 Oct 2025). Lot BW638. Brand new with original box.
 *
 * Style code CQ9545-600 (per box label). Women's training/cross-trainer.
 */
...
const FOLDER = "/tmp/db-batchD/d1-2-legend-essential-2-pink-w95";
const SLUG = "nike-wmns-legend-essential-2-light-soft-pink-w95";
const NAME = "Nike Wmns Legend Essential 2 Light Soft Pink (W 9.5)";
...
async function main() {
  ...
  const heic = path.join(FOLDER, "_receipt", "IMG_5777.HEIC");
  if (existsSync(heic)) {
    console.log("\n🧾 uploading receipt...");
    ...
    if (error) {
      ...
      const { data: receipt, error: rErr } = await supabase
        ...
        .insert({
          ...
          notes:
            "Whatnot packing slip — order #583502138 from fragmented_soles' '500+ PAIRS AT $1 + FREE SHOES' livestream on 15 October 2025. Listed as 'WMNS NIKE LEGEND ESSENTIAL 2 LIGHT SOFT PINK — SIZE 9.5W BRAND NEW ORIGINAL BOX — BW638'. Won for $23.",
        })
        .select("id")
        .single();
      if (!rErr && receipt) {
